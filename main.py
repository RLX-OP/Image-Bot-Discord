...
@slash.slash(
    ...
    options=[
        create_option(
            name="search", description="search an image", required=True, option_type=3
        )
    ],
)
async def image(ctx: SlashContext, *, search: str):
    ...
    result = resource.list(
        q=f"{search}", cx="013d682ac094eda30", searchType="image"
    ).execute()
    url = result["items"][ran]["link"]
    embed = Embed(title=f"Here Is Your Search Result For **({search.title()})**")
